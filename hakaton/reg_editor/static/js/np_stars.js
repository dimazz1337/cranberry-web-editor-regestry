...
function initStars() {
  for (let i = starsCount; i--;) {
    stars.push({
      posX: randIn(0, canvas.a.width),
      posY: randIn(0, canvas.a.height),
      color: `rgba(255, 255, 255, ${randIn(0, 1)})`,
    });
  }
};
...
function updateStars(i) {
  stars[i].posX = stars[i].posX + .3 > canvas.a.width ? 0 : stars[i].posX + .3;
  stars[i].posY = stars[i].posY + .1 > canvas.a.height ? 0 : stars[i].posY + .1;

  drawStar(stars[i].posX, stars[i].posY, stars[i].color);
}

function drawStar(x, y, color) {
  ctx.b.save();
  ctx.b.beginPath();
  ctx.b.fillStyle = color;
  ctx.b.arc(x, y, 1, 0, TAU, false);
  ctx.b.fill();
  ctx.b.closePath();
  ctx.b.restore();
}
...
function drawRay(x, y1, y2, life, ttl, width, hue) {
  ...
  gradient.addColorStop(0.5, `hsla(${hue*1.5},100%,60%,${fadeInOut(life, ttl)})`);
  gradient.addColorStop(1, `hsla(${hue*1.5},10%,65%,0)`);
